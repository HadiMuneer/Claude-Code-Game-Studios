...
if [ "$SRC_FILES" -gt 50 ] && [ "$DESIGN_FILES" -lt 5 ]; then
  echo "WARNING GAP: Substantial codebase ($SRC_FILES source files) but sparse design docs ($DESIGN_FILES files)"
...
if [ -d "prototypes" ]; then
  ...
  if [ -n "$PROTOTYPE_DIRS" ]; then
    ...
    if [ ${#UNDOCUMENTED_PROTOS[@]} -gt 0 ]; then
      echo "WARNING GAP: ${#UNDOCUMENTED_PROTOS[@]} undocumented prototype(s) found:"
...
if [ -d "src/core" ] || [ -d "src/engine" ]; then
  if [ ! -d "docs/architecture" ]; then
    echo "WARNING GAP: Core engine/systems exist but no docs/architecture/ directory"
    ...
    if [ "$ADR_COUNT" -lt 3 ]; then
      echo "WARNING GAP: Core systems exist but only $ADR_COUNT ADR(s) documented"
    ...
    while IFS= read -r system_dir; do
      ...
      if [ "$file_count" -ge 5 ]; then
        ...
        if [ ! -f "$design_doc_1" ] && [ ! -f "$design_doc_2" ]; then
          echo "WARNING GAP: Gameplay system 'src/gameplay/$system_name/' ($file_count files) has no design doc"
...
if [ "$SRC_FILES" -gt 100 ]; then
  ...
  if [ ! -d "production/sprints" ] && [ ! -d "production/milestones" ]; then
    echo "WARNING GAP: Large codebase ($SRC_FILES files) but no production planning found"
